(function () {
  ...
  function shell() {
    app.innerHTML = `<div class="admin-shell"><aside class="sidebar"><p class="kicker">OPERATIONS</p><h1>YEONJO</h1><nav><button class="active" data-view="dashboard">대시보드</button><button data-view="orders">주문·배송</button><button data-view="members">회원관리</button><button data-view="settings">운영설정</button></nav><a href="index.html">쇼핑몰 보기 ↗</a><button id="adminLogout">로그아웃</button></aside><section class="content" id="content"></section></div>`;
    document.querySelectorAll('[data-view]').forEach(button => button.onclick = () => {
      document.querySelectorAll('[data-view]').forEach(item => item.classList.remove('active'));
      button.classList.add('active');
      ({ dashboard, orders, members, settings }[button.dataset.view])();
    });
